...
func (w *Watcher) StartWatcher() error {
	...
	// Allocate a fresh stop channel so the watcher can be stopped and restarted.
	// StopWatcher closes this channel, so a new one is needed on each start.
...
func (w *Watcher) StopWatcher() error {
	...
	// Signal the fsnotify and backup goroutines to exit. stopChan is only ever
	// closed here, and the fsnotifyWatcher nil-check above guards against a
	// second close.
	close(w.stopChan)
